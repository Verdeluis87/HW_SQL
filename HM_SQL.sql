...
SELECT f.title AS Film, COUNT(fa.film_id) AS number_of_actors
FROM film f
...
SELECT f.title
FROM film f
WHERE f.title LIKE 'K%' OR f.title LIKE 'Q%' AND language_id IN
( 
	SELECT language_id
	FROM language l
...
SELECT store.store_id, SUM(payment.amount) AS total_store
FROM store
...
SELECT category.name, SUM(payment.amount) AS total_category
FROM category
